      ...
      {result && (
        <div className="w-full flex-1 overflow-y-auto custom-scrollbar p-4 md:p-6 space-y-6 animate-in slide-in-from-bottom-8 duration-700 pb-20 flex flex-col items-center">
          <div className="w-full max-w-3xl md:max-w-4xl bg-white dark:bg-slate-900 rounded-2xl md:rounded-3xl shadow-xl dark:shadow-2xl border border-slate-200/50 dark:border-slate-800 overflow-hidden">
            <div className="bg-gradient-to-br from-indigo-600 to-violet-700 p-6 md:p-8 text-white flex flex-col sm:flex-row justify-between items-center gap-4 md:gap-6 relative overflow-hidden">
              ...
              <div className="relative z-10 text-center sm:text-left">
                ...
                <p className="text-indigo-100 text-xs mt-2 font-bold opacity-70 tracking-[0.15em] uppercase truncate">{aiConfig.model}</p>
              </div>
            ...
            <div className="p-6 md:p-8 space-y-8">
              <section>
                <div className="flex items-center gap-3 mb-5">
                   ...
                   <h4 className="text-xs font-black text-indigo-500 uppercase tracking-[0.3em]">{t.summary}</h4>
                </div>
              ...
              <section>
                <div className="flex items-center gap-3 mb-5">
                   <div className="w-2 h-2 rounded-full bg-indigo-500 shadow-[0_0_12px_rgba(99,102,241,0.8)]" />
                   <h4 className="text-xs font-black text-indigo-500 uppercase tracking-[0.3em]">{t.improvements}</h4>
                </div>
                <div className="grid grid-cols-1 gap-4">
                  {result.suggestions.map((s, i) => (
                    <div key={i} className="flex items-start gap-4 bg-slate-50/50 dark:bg-slate-800/30 p-4 md:p-5 rounded-2xl border border-slate-200/80 dark:border-slate-800/80 hover:bg-white dark:hover:bg-slate-800 hover:shadow-lg transition-all group">
                      <div className="w-8 h-8 rounded-xl bg-white dark:bg-slate-900 text-indigo-600 font-black flex items-center justify-center text-sm shadow-md group-hover:bg-indigo-600 group-hover:text-white transition-all shrink-0 border border-slate-200 dark:border-slate-800">
                        {i + 1}
                      </div>
            ...
            <div className="bg-slate-50/50 dark:bg-slate-800/20 p-6 border-t border-slate-200 dark:border-slate-800 flex justify-center">
              <button
                onClick={handleAnalyze}
                className="flex items-center gap-3 px-8 py-3 rounded-xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 text-slate-600 dark:text-slate-400 hover:text-indigo-600 hover:border-indigo-500/30 transition-all font-black text-xs uppercase tracking-[0.2em] shadow-md hover:shadow-lg"
              >
                {loading ? (
                  <div className="w-4 h-4 border-3 border-indigo-200 border-t-indigo-600 rounded-full animate-spin" />
                ) : (
