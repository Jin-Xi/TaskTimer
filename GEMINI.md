...
This project uses the **OpenSpec** workflow for managing changes.
- Active Change: `motion-polish-2025`
- Skills available: `/opsx:propose`, `/opsx:explore`, `/opsx:apply`, `/opsx:archive`
- Changes are tracked in the `openspec/changes/` directory.

## 行为规范

- 你可以随时使用 `npm run dev` 启动服务来验证你的修改。但是，在你完成任务并向我输出最终结果之前，你必须终止该开发服务器的进程。绝对不能让它在后台持续运行。
- 在执行任何写操作或 shell 命令之前，必须先解释你的意图。
- 遵循现有的代码风格和架构模式。
- 优先进行微小的、手术式的更改。
- 更改后必须进行验证（运行测试、Lint 等）。
